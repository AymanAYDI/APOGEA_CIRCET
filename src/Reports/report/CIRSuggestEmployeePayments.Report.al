report 50029 "CIR Suggest Employee Payments"
{
        ...
        layout
        {
            area(content)
            {
                group(Options)
                {
                    ...
                    group("Find Payments")
                    {
                        Caption = 'Find Payments', Comment = '{"instructions":"","translations":[{"lang":"FRA","txt":"Rechercher les paiements"}]}';
                        field("Available Amount (LCY)"; AmountAvailable)
                        {
                            ...
                            ToolTip = 'Specifies a maximum amount (in LCY) that is available for payments.', Comment = '{"instructions":"","translations":[{"lang":"FRA","txt":"Spécifie un montant maximal (en DS) qui est disponible pour les paiements."}]}';
                        }
                        field(SkipExportedPayments; VarSkipExportedPayments)
                        {
                            ApplicationArea = Basic, Suite;
                            Caption = 'Skip Exported Payments', Comment = '{"instructions":"","translations":[{"lang":"FRA","txt":"Ignorer les paiements exportés"}]}';
                            Importance = Additional;
                            ToolTip = 'Specifies if you do not want the batch job to insert payment journal lines for documents for which payments have already been exported to a bank file.', Comment = '{"instructions":"","translations":[{"lang":"FRA","txt":"Indique si vous ne souhaitez pas que le traitement par lots insère les lignes feuille paiement pour les documents pour lesquels les paiements ont déjà été exportés vers un fichier bancaire."}]}';
                        }
                    }
                    group("Summarize Results")
                    {
                        Caption = 'Summarize Results', Comment = '{"instructions":"","translations":[{"lang":"FRA","txt":"Résumer les résultats"}]}';
                        field(SummarizePerEmployee; SummarizePerEmpl)
                        {
                            ApplicationArea = Basic, Suite;
                            Caption = 'Summarize per Employee', Comment = '{"instructions":"","translations":[{"lang":"FRA","txt":"Totaliser par salarié"}]}';
                            ToolTip = 'Specifies if you want the batch job to make one line per employee', Comment = '{"instructions":"","translations":[{"lang":"FRA","txt":"Spécifie si vous souhaitez que le traitement par lots crée une ligne par salarié"}]}';
                        }
                        field(NegativBalance; NegatieBalance)
                        {
                            ApplicationArea = Basic, Suite;
                            Caption = 'Include Negative Balance', Comment = '{"instructions":"","translations":[{"lang":"FRA","txt":"Inclure les soldes des salariés négatifs"}]}';
                            ToolTip = 'Specifies the value of the Include Negative Balance ', Comment = '{"instructions":"","translations":[{"lang":"FRA","txt":"Inclure les soldes des salariés négatifs"}]}';
                        }
                        field(SummarizePerDimText; VarSummarizePerDimText)
                        {
                            ApplicationArea = Dimensions;
                            Caption = 'By Dimension', Comment = '{"instructions":"","translations":[{"lang":"FRA","txt":"Par axe"}]}';
                            Editable = false;
                            Enabled = SummarizePerDimTextEnable;
                            Importance = Additional;
                            ToolTip = 'Specifies the dimensions that you want the batch job to consider.', Comment = '{"instructions":"","translations":[{"lang":"FRA","txt":"Spécifie les axes analytiques dont vous souhaitez que le traitement par lots tienne compte."}]}';

                            trigger OnAssistEdit()
                            var
                                DimSelectionBuf: Record "Dimension Selection Buffer";
                            begin
                                DimSelectionBuf.SetDimSelectionMultiple(3, REPORT::"Suggest Employee Payments", VarSummarizePerDimText);
                            end;
                        }
                    }
                    group("Fill in Journal Lines")
                    {
                        Caption = 'Fill in Journal Lines', Comment = '{"instructions":"","translations":[{"lang":"FRA","txt":"Renseigner les lignes feuille"}]}';
                        field(PostingDate; VarPostingDate)
                        {
                            ApplicationArea = Basic, Suite;
                            Caption = 'Posting Date', Comment = '{"instructions":"","translations":[{"lang":"FRA","txt":"Date comptabilisation"}]}';
                            Importance = Promoted;
                            ToolTip = 'Specifies the date for the posting of this batch job. By default, the working date is entered, but you can change it.', Comment = '{"instructions":"","translations":[{"lang":"FRA","txt":"Spécifie la date comptabilisation de ce traitement par lots. Par défaut, la date de travail est saisie, mais vous pouvez la modifier."}]}';
                        ...
                        {
                            ApplicationArea = Basic, Suite;
                            Caption = 'Starting Document No.', Comment = '{"instructions":"","translations":[{"lang":"FRA","txt":"N° document début"}]}';
                            ToolTip = 'Specifies the next available number in the number series for the journal batch that is linked to the payment journal. When you run the batch job, this is the document number that appears on the first payment journal line. You can also fill in this field manually.', Comment = '{"instructions":"","translations":[{"lang":"FRA","txt":"Spécifie le numéro suivant disponible dans la souche de numéros pour le nom feuille associé à la feuille paiements. Lorsque vous exécutez le traitement par lots, il s''agit du numéro de document qui s''affiche sur la première ligne feuille paiements. Vous pouvez également compléter ce champ manuellement."}]}';
                        ...
                        {
                            ApplicationArea = Basic, Suite;
                            Caption = 'New Doc. No. per Line', Comment = '{"instructions":"","translations":[{"lang":"FRA","txt":"Nouveau n° document par ligne"}]}';
                            Importance = Additional;
                            ToolTip = 'Specifies if you want the batch job to fill in the payment journal lines with consecutive document numbers, starting with the document number specified in the Starting Document No. field.', Comment = '{"instructions":"","translations":[{"lang":"FRA","txt":"Spécifie si vous souhaitez que le traitement par lots insère des numéros de document consécutifs, commençant par le numéro de document spécifié dans le champ Document de départ n°, sur les lignes feuille paiements."}]}';
                        }
                        field(BalAccountType; GenJnlLine2."Bal. Account Type")
                        {
                            ApplicationArea = Basic, Suite;
                            Caption = 'Bal. Account Type', Comment = '{"instructions":"","translations":[{"lang":"FRA","txt":"Type compte contrepartie"}]}';
                            Importance = Additional;
                            ToolTip = 'Specifies the balancing account type that payments on the payment journal are posted to.', Comment = '{"instructions":"","translations":[{"lang":"FRA","txt":"Spécifie le type de compte contrepartie dans lequel les paiements de la feuille paiements sont validés."}]}';
                        ...
                        {
                            ApplicationArea = Basic, Suite;
                            Caption = 'Bal. Account No.', Comment = '{"instructions":"","translations":[{"lang":"FRA","txt":"N° compte contrepartie"}]}';
                            Importance = Additional;
                            ToolTip = 'Specifies the balancing account number that payments on the payment journal are posted to.', Comment = '{"instructions":"","translations":[{"lang":"FRA","txt":"Spécifie le numéro de compte contrepartie dans lequel les paiements de la feuille paiements sont validés."}]}';
                        ...
                        {
                            ApplicationArea = Basic, Suite;
                            Caption = 'Bank Payment Type', Comment = '{"instructions":"","translations":[{"lang":"FRA","txt":"Mode émission paiement"}]}';
                            Importance = Additional;
                            ToolTip = 'Specifies the check type to be used, if you use Bank Account as the balancing account type.', Comment = '{"instructions":"","translations":[{"lang":"FRA","txt":"Spécifie le type de chèque à utiliser si vous utilisez Compte bancaire comme type de compte contrepartie."}]}';
    ...
    var
        ...
        TempOldEmplPaymentBuffer: Record "Employee Payment Buffer" temporary;
        ...
        NegatieBalance: Boolean;
        VarSummarizePerDimText: Text[250];
        LastLineNo: Integer;
        NextEntryNo: Integer;
        ...
        PostingDateRequiredErr: Label 'In the Posting Date field, specify the date that will be used as the posting date for the journal entries.', Comment = '{"instructions":"","translations":[{"lang":"FRA","txt":"Dans le champ Date comptabilisation, spécifiez la date qui sera utilisée comme date de comptabilisation pour les écritures du journal."}]}';
        StartingDocNoErr: Label 'In the Starting Document No. field, specify the first document number to be used.', Comment = '{"instructions":"","translations":[{"lang":"FRA","txt":"Dans le champ N° document début, spécifiez le premier numéro de document à utiliser."}]}';
        ProcessingEmployeesMsg: Label 'Processing employees     #1##########', Comment = '{"instructions":"","translations":[{"lang":"FRA","txt":"Traitement des salariés          #1##########"}]}';
        InsertingJournalLinesMsg: Label 'Inserting payment journal lines #1##########', Comment = '{"instructions":"","translations":[{"lang":"FRA","txt":"Insertion des lignes f. paiement #1##########"}]}';
        AccountTypeErr: Label '%1 must be G/L Account or Bank Account.', Comment = '{"instructions":"","translations":[{"lang":"FRA","txt":"%1 doit être un compte général ou un compte bancaire."}]}';
        BankPaymentTypeErr: Label 'Bank Payment Type field must be filled only when Bal. Account Type is set to Bank Account.', Comment = '{"instructions":"","translations":[{"lang":"FRA","txt":"Le champ Mode émission paiement doit être renseigné uniquement lorsque Type compte contrepartie est défini sur Compte bancaire."}]}';
        BalAccountTypeErr: label 'Balancing account must be %1 or %2.', Comment = '{"instructions":"","translations":[{"lang":"FRA","txt":"Le compte contrepartie doit être %1 ou %2."}]}';
        ManualCheckErr: Label 'If bank payment type is set to Manual Check, and you have not selected the Summarize per Employee field,\ then you must select the New Doc. No. per Line.', Comment = '{"instructions":"","translations":[{"lang":"FRA","txt":"Si le type de paiement bancaire est défini sur Manuel et que vous n’avez pas sélectionné le champ Totaliser par salarié,\\ vous devez alors sélectionner Nouveau n° document par ligne."}]}';
        EmployeePaymentLinesCreatedTxt: Label 'You have created suggested employee payment lines.', Comment = '{"instructions":"","translations":[{"lang":"FRA","txt":"Vous avez créé des propositions de lignes de paiement salarié."}]}';
        UnprocessedEntriesQst: Label 'There are one or more entries for which no payment suggestions have been made because the posting dates of the entries are later than the requested posting date. Do you want to see the entries?', Comment = '{"instructions":"","translations":[{"lang":"FRA","txt":"Il existe une ou plusieurs écritures pour lesquelles aucune suggestion de paiement n''a été faite car les dates de comptabilisation de ces écritures sont postérieures à celle demandée. Voulez-vous visualiser ces écritures ?"}]}';
        SummarizePerDimTextEnable: Boolean;
        ShowPostingDateWarning: Boolean;
        EmployeeBalance: Decimal;
        ReplacePostingDateMsg: Label 'For one or more entries, the requested posting date is before the work date.\\These posting dates will use the work date.', Comment = '{"instructions":"","translations":[{"lang":"FRA","txt":"Pour une ou plusieurs écritures, la date de comptabilisation demandée est antérieure à la date de travail.\\Ces dates de comptabilisation utiliseront la date de travail."}]}';
        VarSkipExportedPayments: Boolean;
        StartingDocumentNoErr: Label 'The value in the Starting Document No. field must have a number so that we can assign the next number in the series.', Comment = '{"instructions":"","translations":[{"lang":"FRA","txt":"La valeur du champ N° document début doit contenir un numéro afin que nous puissions attribuer le numéro suivant dans la souche de numéros."}]}';
        UnsupportedCurrencyErr: Label 'The balancing bank account must have local currency.', Comment = '{"instructions":"","translations":[{"lang":"FRA","txt":"Le compte bancaire de contrepartie doit être en devise société."}]}';
    ...
    begin
        ...
        if GenJnlBatch."No. Series" = '' then
            ...
            NextDocNo := NoSeriesMgt.GetNextNo(GenJnlBatch."No. Series", VarPostingDate, false);
            Clear(NoSeriesMgt);
        end;
    end;

    procedure InitializeRequest(NewAvailableAmount: Decimal; NewSkipExportedPayments: Boolean; NewPostingDate: Date; NewStartDocNo: Code[20]; NewSummarizePerEmpl: Boolean; pBalAccType: Enum "Gen. Journal Account Type"; pBalAccNo: Code[20]; pBankPmtType: Enum "Bank Payment Type")
    begin
        AmountAvailable := NewAvailableAmount;
        VarSkipExportedPayments := NewSkipExportedPayments;
        VarPostingDate := NewPostingDate;
        NextDocNo := NewStartDocNo;
        SummarizePerEmpl := NewSummarizePerEmpl;
        GenJnlLine2."Bal. Account Type" := pBalAccType;
        GenJnlLine2."Bal. Account No." := pBalAccNo;
        GenJnlLine2."Bank Payment Type" := pBankPmtType;
    end;

    local procedure GetEmplLedgEntries(Positive: Boolean)
    begin
        EmployeeLedgerEntry.Reset();
        EmployeeLedgerEntry.CopyFilters("Employee Ledger Entry");
        ...
        if VarSkipExportedPayments then
    ...
    begin
        GenJnlLine.Init();
        GenJnlLine.Validate("Posting Date", VarPostingDate);
...
#pragma warning restore AA0205
    ...
    begin
        Clear(TempOldEmplPaymentBuffer);
            ...
            repeat
                ...
                if SummarizePerEmpl then
                    ...
                    if DocNoPerLine then begin
                        ...
                        if (TempEmplPaymentBuffer."Employee No." = TempOldEmplPaymentBuffer."Employee No.") and
                           (TempEmplPaymentBuffer."Currency Code" = TempOldEmplPaymentBuffer."Currency Code")
                        then
                            GenJnlLine."Document No." := TempOldEmplPaymentBuffer."Document No."
                        else begin
                            GenJnlLine."Document No." := NextDocNo;
                            GenJnlLine.IncrementDocumentNo(GenJnlBatch, NextDocNo);
                            TempOldEmplPaymentBuffer := TempEmplPaymentBuffer;
                            TempOldEmplPaymentBuffer."Document No." := GenJnlLine."Document No.";
                        end;
                GenJnlLine."Account Type" := GenJnlLine."Account Type"::Employee;
    ...
    local procedure UpdateDimensions(var pGenJnlLine: Record "Gen. Journal Line")
    var
        DimBuf: Record "Dimension Buffer";
    ...
    begin
        NewDimensionID := pGenJnlLine."Dimension Set ID";
        if SummarizePerEmpl then begin
            DimBuf.Reset();
            DimBuf.DeleteAll();
            ...
            pGenJnlLine."Dimension Set ID" := NewDimensionID;
        end;
        DimMgt.AddDimSource(DefaultDimSource, DimMgt.TypeToTableID1(pGenJnlLine."Account Type".AsInteger()), pGenJnlLine."Account No.");
        DimMgt.AddDimSource(DefaultDimSource, DimMgt.TypeToTableID1(pGenJnlLine."Bal. Account Type".AsInteger()), pGenJnlLine."Bal. Account No.");
        DimMgt.AddDimSource(DefaultDimSource, Database::Job, pGenJnlLine."Job No.");
        DimMgt.AddDimSource(DefaultDimSource, Database::"Salesperson/Purchaser", pGenJnlLine."Salespers./Purch. Code");
        DimMgt.AddDimSource(DefaultDimSource, Database::Campaign, pGenJnlLine."Campaign No.");
        pGenJnlLine.CreateDim(DefaultDimSource);
        if NewDimensionID <> pGenJnlLine."Dimension Set ID" then begin
            DimSetIDArr[2] := NewDimensionID;
            DimSetIDArr[1] := pGenJnlLine."Dimension Set ID";
            pGenJnlLine."Dimension Set ID" :=
              DimMgt.GetCombinedDimensionSetID(DimSetIDArr, pGenJnlLine."Shortcut Dimension 1 Code", pGenJnlLine."Shortcut Dimension 2 Code");
        end;

        if SummarizePerEmpl then begin
            DimMgt.GetDimensionSet(TempDimSetEntry, pGenJnlLine."Dimension Set ID");
            if AdjustAgainstSelectedDim(TempDimSetEntry, TempDimSetEntry2) then
                pGenJnlLine."Dimension Set ID" := DimMgt.GetDimensionSetID(TempDimSetEntry2);
            DimMgt.UpdateGlobalDimFromDimSetID(pGenJnlLine."Dimension Set ID", pGenJnlLine."Shortcut Dimension 1 Code",
              pGenJnlLine."Shortcut Dimension 2 Code");
        end;
    end;
    ...
    local procedure CheckCurrencies(pBalAccType: Enum "Gen. Journal Account Type"; pBalAccNo: Code[20])
    var
        lBankAcc: Record "Bank Account";
    begin
        if pBalAccType = pBalAccType::"Bank Account" then
            if pBalAccNo <> '' then begin
                lBankAcc.Get(pBalAccNo);
                if lBankAcc."Currency Code" <> '' then
